...
function lang_predicate(langCode: string): TFilePredicate {
    if (langCode) {
        REX_LANG_CODE.lastIndex = 0;
        if (!REX_LANG_CODE.exec(langCode)) throw new Error(langCode + " no tiene un formato de idioma válido(xx_XX).")
...
function langs(directories: string | string[], langCode?: string | ILangOptions, options?: ILangOptions): TMessages {
    ...
    if (typeof langCode === 'string') {
        ...
        options = <ILangOptions>langCode;
    }

    const msgs: TMessageDictionaty = {};
    const langs: path.ParsedPath[] = [];
    let assign: typeof safe_assign;
    if (options.safe === undefined) {
        assign = (process.env.NODE_ENV === 'production') ?
            unsafe_assign :
            safe_assign;
    } else {
        assign = options.safe ?
    ...
    for (let dir of directories) {
        for (let it of file_gen(dir, lang_predicate(options.langCode))) {
            if (it.base !== _defaultFileName) {
                ...
                assign(msgs, JSON.parse(fs.readFileSync(path.format(it), 'utf8')));
            }
        }
